...
func _save_function(world_scene: Node3D, save_file_number: int):
	...
	new_config.set_value("Globalvaribles", "game list", PurchasableItemList.games_list)
	new_config.set_value("Globalvaribles", "decoration list", PurchasableItemList.decoration_list)
	new_config.set_value("Globalvaribles", "spell list", PurchasableItemList.spell_list)
	...
	new_config.set_value("Globalvaribles", "tutorials", TutorialManager.tutorials)
...
func _load_function(save_file_number: int):
	...
	if loading == OK:
		...
		PurchasableItemList.games_list = new_config.get_value("Globalvaribles", "game list")
		PurchasableItemList.decoration_list = new_config.get_value("Globalvaribles", "decoration list")
		PurchasableItemList.spell_list = new_config.get_value("Globalvaribles", "spell list")
		Globals.bar_unlock = new_config.get_value("Globalvaribles", "bar_unlock")
		Globals.ring_unlock = new_config.get_value("Globalvaribles", "ring_unlock")
		TutorialManager.tutorials = new_config.get_value("Globalvaribles", "tutorials")
